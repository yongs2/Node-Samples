...
class JsonSocket extends Duplex {
  ...
  _onReadable() {
    ...
    while (!this._readingPaused) {
      ...
      let lenBuf = this._socket.read(MAX_HEADER_SIZE);
      if (!lenBuf) return;
      ...
      let strHeaderStart = lenBuf.slice(0, 4).toString();
      let len = lenBuf.slice(4, MAX_HEADER_SIZE).toString();
  ...
  _write(obj, encoding, cb) {
    ...
    var header = sprintf("FEFE%012d", json.length);
    let headerBytes = Buffer.byteLength(header);
    let jsonBytes = Buffer.byteLength(json);
    let buffer = Buffer.alloc(headerBytes + jsonBytes);
    buffer.write(header);
    buffer.write(json, MAX_HEADER_SIZE);
